...
void qsort(void *v[], int left, int right, int (*comp) (void *, void *)) { // Funcion qsort, no retorna nada; recibe array de punteros, dos indices, y un apuntador a funcion que retorna entero, y que recibe dos punteros genericos
    int i, last;
    ...
    qsort(v, left, last-1, comp); // Iniciar la recursion al lado izquierdo, de los menores al elemento medio, entregando la direccion de la funcion a utilizar.
    qsort(v, last + 1, right, comp); // Iniciar la recursion al lado derecho, de los mayores al elemento medio, entregando la direccion de la funcion a utilizar.    
}
...
int main (int argc, char *argv[]) {
    ...
    if (argc > 1 && strcmp(argv[1], "-n") == 0) // Si hay más de un argumento y el adicional es la opcion -n, poner verdadero al bool de comparacion numerica.
        numeric = 1;
